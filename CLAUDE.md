...
**2. Room schema is versioned and migrating.** `AppDatabase` is at version 4 with explicit `MIGRATION_1_2`, `MIGRATION_2_3` (both add nullable columns to `cached_foods`), and `MIGRATION_3_4` (creates `weight_entries`). Any entity change needs a new migration — don't bump the version without one. Four entities: `LogEntry`, `CachedFood`, `SavedMeal`, `WeightEntry`. Note `exportSchema = false`, so there are no schema JSONs and migrations are not covered by `MigrationTestHelper`; verify migration DDL by matching `Converters` (LocalDate→TEXT, Instant→INTEGER, Float→REAL) by review.
...
**Daily reset and reminders.** `CalorieApp.onCreate` calls `WorkScheduler.scheduleDailyReset(this, 0)` on every launch. `DailyResetWorker` deletes `log_entries` before *yesterday* (so yesterday is preserved for "Copy yesterday"); it does **not** touch `weight_entries`, which are retained indefinitely. `ReminderWorker` fires the optional unlogged-meal reminder. App is `Configuration.Provider` and uses `HiltWorkerFactory` — workers must be `@HiltWorker` with `@AssistedInject`.
